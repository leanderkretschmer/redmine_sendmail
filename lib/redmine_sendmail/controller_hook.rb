...
module RedmineSendmail
  class ControllerHook < Redmine::Hook::Listener
    ...
    def controller_issues_new_after_save(context = {})
      issue  = context[:issue]
      params = context[:params]
      Rails.logger.info("[redmine_sendmail] new_after_save hook fired (issue=#{issue&.id}, params_keys=#{params&.keys&.inspect})")
      unless issue && params
        Rails.logger.info("[redmine_sendmail] new_after_save: skip (missing issue/params)")
        return
      end
      sm = params[:sendmail]
      if sm.blank?
        Rails.logger.info("[redmine_sendmail] new_after_save: skip (no sendmail params submitted)")
        return
      end
      project = issue.project
      unless project&.module_enabled?(:sendmail)
        Rails.logger.info("[redmine_sendmail] new_after_save: skip (module not enabled on project ##{project&.id})")
        return
      end
      unless User.current.allowed_to?(:send_sendmail, project)
        Rails.logger.info("[redmine_sendmail] new_after_save: skip (user #{User.current.login} lacks :send_sendmail)")
        return
      end
      sm_hash = sm.respond_to?(:to_unsafe_h) ? sm.to_unsafe_h : sm.to_h
      att_param = params[:attachments]
      ...
      Rails.logger.info("[redmine_sendmail] new_after_save: enqueueing dispatch for issue ##{issue.id}, params=#{sm_hash.inspect}, attachments=#{att_hash.is_a?(Hash) ? att_hash.keys.inspect : 'none'}")
      RedmineSendmailDispatchJob.perform_later(
        issue_id:          issue.id,
        params:            sm_hash.deep_stringify_keys,
        attachment_params: att_hash.is_a?(Hash) ? att_hash.deep_stringify_keys : nil
      ...
      Rails.logger.error("[redmine_sendmail] new dispatch hook failed: #{e.class}: #{e.message}\n#{e.backtrace.first(5).join("\n")}")
    end
  end
end
